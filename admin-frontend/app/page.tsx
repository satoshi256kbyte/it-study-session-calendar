...
export default function AdminHome() {
  const { user, loading, signOut, signInWithHostedUI, isAuthenticated, error } =
    useAuth()
  ...
  const fetchSessions = async () => {
    try {
      ...
      // 承認待ち件数を計算
      const pendingSessionsCount = sessionData.filter(
        (session: StudySession) => session.status === 'pending'
      ).length
      setPendingCount(pendingSessionsCount)
    } catch (error) {
  ...
  const handleAction = async (
    ...
    try {
      const response = await fetch(
        `/api/admin/study-sessions/${id}/${action}`,
        {
          method: 'POST',
        }
      )
  ...
  const getSessionItemClasses = (session: StudySession) => {
    const baseClasses = 'px-4 py-4 sm:px-6 transition-colors duration-200'
            ...
            {dataLoading ? (
              ...
              <ul className="divide-y divide-gray-200">
                {sessions.map(session => (
                  <li
                    key={session.id}
                    className={getSessionItemClasses(session)}
                  >
                      ...
                      <div className="flex space-x-2 ml-4">
                        {session.status === 'pending' && (
                          <>
                            <button
                              onClick={() =>
                                handleAction(session.id, 'approve')
                              }
